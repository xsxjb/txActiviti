package com.ibusiness.auth.support;
...
import com.ibusiness.auth.manager.UserStatusManager;

import com.ibusiness.security.impl.UserInfoImpl;
import com.ibusiness.security.spi.UserStatusFetcher;
import com.ibusiness.security.spi.UserStatusUpdater;
import com.ibusiness.security.util.UserDetailsBuilder;
...
public class DefaultUserDetailsService implements UserDetailsService, MessageSourceAware {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        UserStatus userStatus = userStatusManager.findUniqueBy("username", username);
        ...
        UserDetails userDetails = new UserDetailsBuilder(userInfo, userStatus.getPassword()).build();
        ...
        if (userStatusFetcher != null) {
            ...
            try {
                ...
                throw new UsernameNotFoundException(messages.getMessage("JdbcDaoImpl.notFound", new Object[] {
                    username }, "Username {0} not found"), ex);
            }
        }
    }
    ...
    protected void loadUserAuthorities(UserStatus userStatus, Set<GrantedAuthority> authsSet) {
        for (Role role : userStatus.getRoles()) {
            for (Perm perm : role.getRoleDef().getPerms()) {
                String text = perm.getName();
                GrantedAuthority grantedAuthority = new SimpleGrantedAuthority(text);
