...
<html>
  ...
  <body>
	...
	<div class="panel panel-default col-md-10"> 
	    <div class="panel-heading"><h4 class="panel-title">经营预算月计划执行表流程控制</h4></div>
	    <div class="panel-body">
	        <div class="pull-left">
	            <a href="#nextTaskUserDiv" role="button" class="btn btn-default btn-sm" data-toggle="modal">办理</a>
			    <button class="btn btn-default btn-sm a-submit" onclick="mainFormSubmit('monthbudgetreal-save-draft.do')">草稿</button>
			    <button class="btn btn-default btn-sm" onclick="location.href='monthbudgetreal-rollback.do?executionId=${model.executionid}&flowId=${flowId}&flowType=0'">回退</button>
			    <button class="btn btn-default btn-sm a-remove" onclick="location.href='monthbudgetreal-list.do?flowId=${flowId}&flowType=0'">返回</button>
			</div>
	   </div>
	    ...
	    <div class="panel-body">
			    <div class="pull-left">
				    <button class="btn btn-default btn-sm a-insert" onclick="location.href='monthbudgetreal_s-input.do?id=${model.id}&subId=&flowId=${flowId}'">新建</button>
		   ...
		   <div class="content">
				<form id="gridForm" name="gridForm" method='post' action="monthbudgetreal_s-remove.do" class="m-form-blank">
				  <input type="hidden" name="flowId" value="${flowId}">
						    ...
						    <tbody>
						      <c:forEach items="${page.result}" var="item">
								  <tr>
									    ...
									    <td>
								            <a href="monthbudgetreal_s-input.do?id=${model.id}&subId=${item.id}&flowId=${flowId}" class="a-update"><spring:message code="core.list.edit" text="编辑"/></a>
								        </td>
							            <td>${item.planyear}</td>
