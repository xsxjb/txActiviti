...
<html>
  <head>
    ...
    <title>经营预算年度计划申报表编辑</title>
    <%@include file="/common/center.jsp"%>
    <script type="text/javascript">
        // 实例化config对象
        var operatbudgetplan_sConfig = {
		    id: 'operatbudgetplan_sCodeGrid',
		    pageNo: ${operatbudgetplan_sPage.pageNo},
		    pageSize: ${operatbudgetplan_sPage.pageSize},
		    totalCount:${operatbudgetplan_sPage.totalCount},
		    resultSize: ${operatbudgetplan_sPage.resultSize},
		    pageCount: ${operatbudgetplan_sPage.pageCount},
		    orderBy: '${operatbudgetplan_sPage.orderBy == null ? '' : operatbudgetplan_sPage.orderBy}',
		    asc: ${operatbudgetplan_sPage.asc},
		    params: {
		        'id': '${model.id}'
		    },
			selectedItemClass: 'operatbudgetplan_sSelectedItem',
			gridFormId: 'operatbudgetplan_sGridForm',
			exportUrl: 'operatbudgetplan_s-export.do'
		};
        // 实例化table对象
		var operatbudgetplan_sTable;
		$(function() {
			operatbudgetplan_sTable = new Table(operatbudgetplan_sConfig);
		    operatbudgetplan_sTable.configPagination('.operatbudgetplan_sM-pagination');
		    operatbudgetplan_sTable.configPageInfo('.operatbudgetplan_sM-page-info');
		    operatbudgetplan_sTable.configPageSize('.operatbudgetplan_sM-page-size');
		});
  ...
  <body>
	...
	<div class="panel panel-default col-lg-10"> 
	    <div class="panel-heading"><h4 class="panel-title glyphicon glyphicon-paperclip">经营预算年度计划申报表流程控制</h4></div>
	    <div class="panel-body">
	        <div class="pull-left">
	            <a href="#nextTaskUserDiv" role="button" class="btn btn-primary btn-sm" data-toggle="modal">办理</a>
			    <button class="btn btn-primary btn-sm a-submit" onclick="mainFormSubmit('operatbudgetplan-save-draft.do')">草稿</button>
			    <button class="btn btn-primary btn-sm" onclick="location.href='operatbudgetplan-rollback.do?executionId=${model.executionid}&flowId=${flowId}&flowType=0'">回退</button>
			    <button class="btn btn-primary btn-sm a-remove" onclick="location.href='operatbudgetplan-list.do?flowId=${flowId}&flowType=0'">返回</button>
			</div>
	   </div>
	    ...
	    <div class="panel-body">
			    <div class="pull-left">
				    <button class="btn btn-primary btn-sm a-insert" onclick="location.href='operatbudgetplan_s-input.do?id=${model.id}&subId=&flowId=${flowId}'"><span class="glyphicon glyphicon-tasks"></span>新建</button>
				    <button class="btn btn-primary btn-sm a-remove" onclick="operatbudgetplan_sTable.removeAll()"><span class="glyphicon glyphicon-trash"></span>删除</button>
	            
				</div>
				<div class="pull-right">
				  每页显示
				  <select class="operatbudgetplan_sM-page-size">
		   ...
		   <div class="content">
				<form id="operatbudgetplan_sGridForm" name="operatbudgetplan_sGridForm" method='post' action="operatbudgetplan_s-remove.do" class="m-form-blank">
				  <input type="hidden" name="flowId" value="${flowId}">
				  <table id="operatbudgetplan_sCodeGrid" class="table table-hover table-striped">
				      <thead>
					      <tr>
						    ...
						    <tbody>
						      <c:forEach items="${operatbudgetplan_sPage.result}" var="item" varStatus="status">
								  <tr class="${status.index%2==1? 'active':''}">
								        <td><input type="checkbox" class="operatbudgetplan_sSelectedItem a-check" name="operatbudgetplan_sSelectedItem" value="${item.id}"></td>
									    <td>
								            <a href="operatbudgetplan_s-input.do?id=${model.id}&subId=${item.id}&flowId=${flowId}" class="a-update"><span class="glyphicon glyphicon-pencil"></span></a>
								        </td>
			  ...
			  <article>
			    <div class="operatbudgetplan_sM-page-info pull-left">
				  共100条记录 显示1到10条记录
				</div>
				<div class="btn-group operatbudgetplan_sM-pagination pull-right">
