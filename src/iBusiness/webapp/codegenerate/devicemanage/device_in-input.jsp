...
<html>
  <head>
    ...
    <title>设备入库流程编辑</title>
    <%@include file="/common/center.jsp"%>
    <script type="text/javascript">
        // 实例化config对象
        var device_in_sConfig = {
		    id: 'device_in_sCodeGrid',
		    pageNo: ${device_in_sPage.pageNo},
		    pageSize: ${device_in_sPage.pageSize},
		    totalCount:${device_in_sPage.totalCount},
		    resultSize: ${device_in_sPage.resultSize},
		    pageCount: ${device_in_sPage.pageCount},
		    orderBy: '${device_in_sPage.orderBy == null ? '' : device_in_sPage.orderBy}',
		    asc: ${device_in_sPage.asc},
		    params: {
		        'id': '${model.id}'
		    },
			selectedItemClass: 'device_in_sSelectedItem',
			gridFormId: 'device_in_sGridForm',
			exportUrl: 'device_in_s-export.do'
		};
        // 实例化table对象
		var device_in_sTable;
		$(function() {
			device_in_sTable = new Table(device_in_sConfig);
		    device_in_sTable.configPagination('.device_in_sM-pagination');
		    device_in_sTable.configPageInfo('.device_in_sM-page-info');
		    device_in_sTable.configPageSize('.device_in_sM-page-size');
		});
  ...
  <body>
	...
	<div class="panel panel-default col-lg-10"> 
	    <div class="panel-heading"><h4 class="panel-title glyphicon glyphicon-paperclip">设备入库流程流程控制</h4></div>
	    <div class="panel-body">
	        <div class="pull-left">
	            <a href="#nextTaskUserDiv" role="button" class="btn btn-primary btn-sm" data-toggle="modal">办理</a>
			    <button class="btn btn-primary btn-sm a-submit" onclick="mainFormSubmit('device_in-save-draft.do')">草稿</button>
			    <button class="btn btn-primary btn-sm" onclick="location.href='device_in-rollback.do?executionId=${model.executionid}&flowId=${flowId}&flowType=0'">回退</button>
			    <button class="btn btn-primary btn-sm a-remove" onclick="location.href='device_in-list.do?flowId=${flowId}&flowType=0'">返回</button>
			</div>
	   </div>
        ...
        <div class="panel-body">
		  <div class="content content-inner">
		       <form id="mainForm" method="post" action="device_in-complete.do" class="form-horizontal">
	    ...
	    <div class="panel-body">
			    <div class="pull-left">
				    <button class="btn btn-primary btn-sm a-insert" onclick="location.href='device_in_s-input.do?id=${model.id}&subId=&flowId=${flowId}'"><span class="glyphicon glyphicon-tasks"></span>新建</button>
				    <button class="btn btn-primary btn-sm a-remove" onclick="device_in_sTable.removeAll()"><span class="glyphicon glyphicon-trash"></span>删除</button>
	            
				</div>
				<div class="pull-right">
				  每页显示
				  <select class="device_in_sM-page-size">
		   ...
		   <div class="content">
				<form id="device_in_sGridForm" name="device_in_sGridForm" method='post' action="device_in_s-remove.do" class="m-form-blank">
				  <input type="hidden" name="flowId" value="${flowId}">
				  <table id="device_in_sCodeGrid" class="table table-hover table-striped">
				      <thead>
					      <tr>
						    ...
						    <tbody>
						      <c:forEach items="${device_in_sPage.result}" var="item" varStatus="status">
								  <tr class="${status.index%2==1? 'active':''}">
								        <td><input type="checkbox" class="device_in_sSelectedItem a-check" name="device_in_sSelectedItem" value="${item.id}"></td>
									    <td>
								            <a href="device_in_s-input.do?id=${model.id}&subId=${item.id}&flowId=${flowId}" class="a-update"><span class="glyphicon glyphicon-pencil"></span></a>
								        </td>
			  ...
			  <article>
			    <div class="device_in_sM-page-info pull-left">
				  共100条记录 显示1到10条记录
				</div>
				<div class="btn-group device_in_sM-pagination pull-right">
