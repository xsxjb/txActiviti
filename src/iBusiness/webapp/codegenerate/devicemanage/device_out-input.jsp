...
<html>
  <head>
    ...
    <title>设备出库信息编辑</title>
    <%@include file="/common/center.jsp"%>
    <script type="text/javascript">
        // 实例化config对象
        var device_out_sConfig = {
		    id: 'device_out_sCodeGrid',
		    pageNo: ${device_out_sPage.pageNo},
		    pageSize: ${device_out_sPage.pageSize},
		    totalCount:${device_out_sPage.totalCount},
		    resultSize: ${device_out_sPage.resultSize},
		    pageCount: ${device_out_sPage.pageCount},
		    orderBy: '${device_out_sPage.orderBy == null ? '' : device_out_sPage.orderBy}',
		    asc: ${device_out_sPage.asc},
		    params: {
		        'id': '${model.id}'
		    },
			selectedItemClass: 'device_out_sSelectedItem',
			gridFormId: 'device_out_sGridForm',
			exportUrl: 'device_out_s-export.do'
		};
        // 实例化table对象
		var device_out_sTable;
		$(function() {
			device_out_sTable = new Table(device_out_sConfig);
		    device_out_sTable.configPagination('.device_out_sM-pagination');
		    device_out_sTable.configPageInfo('.device_out_sM-page-info');
		    device_out_sTable.configPageSize('.device_out_sM-page-size');
		});
  ...
  <body>
	...
	<div class="panel panel-default col-lg-10"> 
	    <div class="panel-heading"><h4 class="panel-title glyphicon glyphicon-paperclip">设备出库信息流程控制</h4></div>
	    <div class="panel-body">
	        <div class="pull-left">
	            <a href="#nextTaskUserDiv" role="button" class="btn btn-primary btn-sm" data-toggle="modal">办理</a>
			    <button class="btn btn-primary btn-sm a-submit" onclick="mainFormSubmit('device_out-save-draft.do')">草稿</button>
			    <button class="btn btn-primary btn-sm" onclick="location.href='device_out-rollback.do?executionId=${model.executionid}&flowId=${flowId}&flowType=0'">回退</button>
			    <button class="btn btn-primary btn-sm a-remove" onclick="location.href='device_out-list.do?flowId=${flowId}&flowType=0'">返回</button>
			</div>
	   </div>
        ...
        <div class="panel-body">
		  <div class="content content-inner">
		       <form id="mainForm" method="post" action="device_out-complete.do" class="form-horizontal">
	    ...
	    <div class="panel-body">
			    <div class="pull-left">
				    <button class="btn btn-primary btn-sm a-insert" onclick="location.href='device_out_s-input.do?id=${model.id}&subId=&flowId=${flowId}'"><span class="glyphicon glyphicon-tasks"></span>新建</button>
				    <button class="btn btn-primary btn-sm a-remove" onclick="device_out_sTable.removeAll()"><span class="glyphicon glyphicon-trash"></span>删除</button>
	            
				</div>
				<div class="pull-right">
				  每页显示
				  <select class="device_out_sM-page-size">
		   ...
		   <div class="content">
				<form id="device_out_sGridForm" name="device_out_sGridForm" method='post' action="device_out_s-remove.do" class="m-form-blank">
				  <input type="hidden" name="flowId" value="${flowId}">
				  <table id="device_out_sCodeGrid" class="table table-hover table-striped">
				      <thead>
					      <tr>
						    ...
						    <tbody>
						      <c:forEach items="${device_out_sPage.result}" var="item" varStatus="status">
								  <tr class="${status.index%2==1? 'active':''}">
								        <td><input type="checkbox" class="device_out_sSelectedItem a-check" name="device_out_sSelectedItem" value="${item.id}"></td>
									    <td>
								            <a href="device_out_s-input.do?id=${model.id}&subId=${item.id}&flowId=${flowId}" class="a-update"><span class="glyphicon glyphicon-pencil"></span></a>
								        </td>
							            <td>${item.deviceno}</td>
							            <td>${item.devicename}</td>
							            <td>${item.devicemodel}</td>
							            <td>${item.deviceunit}</td>
			  ...
			  <article>
			    <div class="device_out_sM-page-info pull-left">
				  共100条记录 显示1到10条记录
				</div>
				<div class="btn-group device_out_sM-pagination pull-right">
