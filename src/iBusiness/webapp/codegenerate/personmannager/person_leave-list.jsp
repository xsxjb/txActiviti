...
<html>
  ...
  <head>
    ...
    <script type="text/javascript">
		var config = {
			...
			gridFormId: 'gridForm',
	        exportUrl: 'person_leave-export.do'
		};
  ...
  <body>
	...
	<!-- 查询条件 -->
        <div class="panel-heading"><h4 class="panel-title glyphicon glyphicon-paperclip">查询</h4></div>
          <div class="panel-body">
	          <div id="search" class="content content-inner">
				  <form name="cgForm" method="post" action="person_leave-list.do" class="form-inline">
	   ...
	   <div class="panel-heading"><h4 class="panel-title glyphicon glyphicon-paperclip">员工离职列表</h4></div>
       <div class="panel-body">
		    <div class="pull-left">
			    <button class="btn btn-primary btn-sm a-insert" href="person_leave-input.do" data-target="#modalInput" data-toggle="modal" data-database="true"><span class="glyphicon glyphicon-tasks"></span>新建</button>
	   ...
	   <div class="content">
			<form id="gridForm" name="gridForm" method='post' action="person_leave-remove.do" class="m-form-blank">
			  <table id="codeGrid" class="table table-hover table-striped">
			      <thead>
				      <tr>
				        <th width="30" class="m-table-check"><input type="checkbox" name="checkAll" onchange="toggleSelectedItems(this.checked)"></th>
					                <th class="sorting">人事档案ID(姓名)</th>
					                <th class="sorting">离退类别</th>
					                <th class="sorting">离职状态</th>
					                <th class="sorting">离退部门</th>
				        <th width="30">&nbsp;</th>
				      </tr>
				    </thead>
					    <tbody>
					      ...
					      <tr class="${status.index%2==1? 'active':''}">
					        <td><input type="checkbox" class="selectedItem a-check" name="selectedItem" value="${item.id}"></td>
						            <td>${item.name}</td>
						            <td>${item.leavesort}</td>
						            <td>${item.leavestate}</td>
						            <td>${item.leavesection}</td>
					        <td>
					          <a href="person_leave-input.do?id=${item.id}" class="a-update" data-target="#modalInput" data-toggle="modal" data-database="true"><span class="glyphicon glyphicon-pencil"></span></a>
					        </td>
					      </tr>
					      </c:forEach>
					    </tbody>
