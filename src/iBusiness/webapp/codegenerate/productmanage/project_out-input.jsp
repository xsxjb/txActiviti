...
<html>
  <head>
    ...
    <script type="text/javascript">
        // 实例化config对象
        var product_out_sConfig = {
		    id: 'product_out_sCodeGrid',
		    pageNo: ${product_out_sPage.pageNo},
		    pageSize: ${product_out_sPage.pageSize},
		    totalCount:${product_out_sPage.totalCount},
		    resultSize: ${product_out_sPage.resultSize},
		    pageCount: ${product_out_sPage.pageCount},
		    orderBy: '${product_out_sPage.orderBy == null ? '' : product_out_sPage.orderBy}',
		    asc: ${product_out_sPage.asc},
		    params: {
		        'id': '${model.id}'
		    },
			selectedItemClass: 'product_out_sSelectedItem',
			gridFormId: 'product_out_sGridForm',
			exportUrl: 'product_out_s-export.do'
		};
        // 实例化table对象
		var product_out_sTable;
		$(function() {
			product_out_sTable = new Table(product_out_sConfig);
		    product_out_sTable.configPagination('.product_out_sM-pagination');
		    product_out_sTable.configPageInfo('.product_out_sM-page-info');
		    product_out_sTable.configPageSize('.product_out_sM-page-size');
		});
  ...
  <body>
	...
	<div class="panel panel-default col-lg-10"> 
	    <div class="panel-heading"><h4 class="panel-title glyphicon glyphicon-paperclip">项目出库流程控制</h4></div>
	    <div class="panel-body">
	        <div class="pull-left">
	            <a href="#nextTaskUserDiv" role="button" class="btn btn-primary btn-sm" data-toggle="modal">办理</a>
			    <button class="btn btn-primary btn-sm a-submit" onclick="mainFormSubmit('project_out-save-draft.do')">草稿</button>
			    <button class="btn btn-primary btn-sm" onclick="location.href='project_out-rollback.do?executionId=${model.executionid}&flowId=${flowId}&flowType=0'">回退</button>
			    <button class="btn btn-primary btn-sm a-remove" onclick="location.href='project_out-list.do?flowId=${flowId}&flowType=0'">返回</button>
			</div>
	   </div>
	    ...
	    <div class="panel-body">
			    <div class="pull-left">
				    <button class="btn btn-primary btn-sm a-insert" onclick="location.href='product_out_s-input.do?id=${model.id}&subId=&flowId=${flowId}'"><span class="glyphicon glyphicon-tasks"></span>新建</button>
				    <button class="btn btn-primary btn-sm a-remove" onclick="product_out_sTable.removeAll()"><span class="glyphicon glyphicon-trash"></span>删除</button>
				    <button class="btn btn-primary btn-sm" onclick="product_out_sTable.exportExcel()">导出Excel</button>
	            
				</div>
				<div class="pull-right">
				  每页显示
				  <select class="product_out_sM-page-size">
		   ...
		   <div class="content">
				<form id="product_out_sGridForm" name="product_out_sGridForm" method='post' action="product_out_s-remove.do" class="m-form-blank">
				  <input type="hidden" name="flowId" value="${flowId}">
				  <table id="product_out_sCodeGrid" class="table table-hover table-striped">
				      <thead>
					      <tr>
						    ...
						    <tbody>
						      <c:forEach items="${product_out_sPage.result}" var="item" varStatus="status">
								  <tr class="${status.index%2==1? 'active':''}">
								        <td><input type="checkbox" class="product_out_sSelectedItem a-check" name="product_out_sSelectedItem" value="${item.id}"></td>
									    <td>
								            <a href="product_out_s-input.do?id=${model.id}&subId=${item.id}&flowId=${flowId}" class="a-update"><span class="glyphicon glyphicon-pencil"></span></a>
								        </td>
							            <td>${item.batchno}</td>
			  ...
			  <article>
			    <div class="product_out_sM-page-info pull-left">
				  共100条记录 显示1到10条记录
				</div>
				<div class="btn-group product_out_sM-pagination pull-right">
