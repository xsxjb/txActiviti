...
OpenLayers.Layer.g = OpenLayers.Class(OpenLayers.Layer.HTTPRequest, {
	...
	timerId : null,
	initialize : function(e, f, g, h) {
		OpenLayers.Layer.HTTPRequest.prototype.initialize
	...
	moveTo : function(f, g, h) {
		...
		if (null != f) {
			...
			this.singleTile ? (i || !h && !j.containsBounds(f))
					&& this.initSingleTile(f) : i || !j.containsBounds(f, !0)
	...
	scheduleMoveGriddedTiles : function() {
		...
		this.timerId = window.setTimeout(this._moveGriddedTiles,
				this.tileLoadingDelay)
	},
	setTileSize : function(b) {
		if (this.singleTile) {
			b = this.map.getSize(), b.h = parseInt(b.h * this.ratio), b.w = parseInt(b.w
					* this.ratio)
		}
	...
	initSingleTile : function(d) {
		var e = d.getCenterLonLat(), f = d.getWidth() * this.ratio, d = d
				.getHeight()
				* this.ratio, e = new OpenLayers.Bounds(e.lon - f / 2, e.lat
						- d / 2, e.lon + f / 2, e.lat + d / 2), f = this.map
				.getLayerPxFromLonLat(new OpenLayers.LonLat(e.left, e.top));
		this.grid.length || (this.grid[0] = []);
		(d = this.grid[0][0]) ? d.moveTo(e, f) : (d = this.addTile(e, f), this
				.addTileMonitoringHooks(d), d.draw(), this.grid[0][0] = d);
	...
	getTileOrigin : function() {
		...
		if (!c) {
			var c = this.getMaxExtent(), d = {
				...
				bl : ["left", "bottom"],
				br : ["right", "bottom"]
			}[this.tileOriginCorner], c = new OpenLayers.LonLat(c[d[0]],
					c[d[1]])
		}
		return c
	},
	initGriddedTiles : function(t) {
		var u = this.map.getSize(), v = Math.ceil(u.h / this.tileSize.h)
				+ Math.max(1, 2 * this.buffer), u = Math.ceil(u.w
				/ this.tileSize.w)
				+ Math.max(1, 2 * this.buffer), w = this.getTileOrigin(), x = this.map
				.getResolution(), y = this.calculateGridLayout(t, w, x), w = Math
				.round(y.tileoffsetx), x = Math.round(y.tileoffsety), A = y.tileoffsetlon, B = y.tileoffsetlat, z = y.tilelon, y = y.tilelat;
		...
		do {
			...
			var A = E, w = C, I = 0;
			do {
				var J = new OpenLayers.Bounds(A, B, A + z, B + y), F = w, F = F
						- K, D = x, D = D - L, F = new OpenLayers.Pixel(F, D);
				(D = G[I++])
						? D.moveTo(J, F, !1)
						: (D = this.addTile(J, F), this
								.addTileMonitoringHooks(D), G.push(D));
				A += z;
				w += this.tileSize.w
	...
	moveGriddedTiles : function() {
		var f = !0, g = this.buffer || 1, h = this.grid[0][0].position, i = parseInt(this.map.layerContainerDiv.style.left), j = parseInt(this.map.layerContainerDiv.style.top), h = h
				.add(i, j);
		h.x > -this.tileSize.w * (g - 1)
				? this.shiftColumn(!0)
				: h.x < -this.tileSize.w * g
						? this.shiftColumn(!1)
						: h.y > -this.tileSize.h * (g - 1)
								? this.shiftRow(!0)
	...
	shiftRow : function(k) {
		for (var l = this.grid, m = l[k ? 0 : this.grid.length - 1], n = this.map
	...
	removeExcessTiles : function(g, h) {
		for (; this.grid.length > g;) {
			for (var i = this.grid.pop(), j = 0, k = i.length; j < k; j++) {
				var l = i[j];
				this.removeTileMonitoringHooks(l);
				l.destroy()
			}
	...
	CLASS_NAME : "OpenLayers.Layer.g"
});
var ea = new OpenLayers.Bounds(-20037508.342787, -20037508.342787,
		20037508.342787, 20037508.342787);
OpenLayers.Layer.z = OpenLayers.Class(OpenLayers.Layer.g, {
	url : null,
	size : null,
	isBaseLayer : !0,
	...
	initializeZLayer : function(c) {
		var c = c.clone(), d = new OpenLayers.Size(Math.ceil(c.w
...
OpenLayers.Layer.k = OpenLayers.Class(OpenLayers.Layer.g, {
	isBaseLayer : !0,
	sphericalMercator : !1,
	zoomOffset : 0,
...
OpenLayers.Layer.m = OpenLayers.Class(OpenLayers.Layer.k, {
	name : "OpenStreetMap",
	...
	wrapDateLine : !0,
	CLASS_NAME : "OpenLayers.Layer.m"
});
var maxExtent = new OpenLayers.Bounds(-20037508.342787, -20037508.342787,
		20037508.342787, 20037508.342787), minExtent = new OpenLayers.Bounds(
		eval(function(p, a, c, k, e, d) {
					...
					while (c--)
						if (k[c])
							p = p
									.replace(new RegExp('\\b' + e(c) + '\\b',
													'g'), k[c]);
					return p;
				}('0.1', 2, 2, '14066100|645973'.split('|'), 0, {})), eval(
						...
						if (k[c])
							p = p
									.replace(new RegExp('\\b' + e(c) + '\\b',
													'g'), k[c]);
					return p;
				}('0.1', 2, 2, '5706683|5599546'.split('|'), 0, {})), eval(
				function(p, a, c, k, e, d) {
					e = function(c) {
						return (c < a ? "" : e(parseInt(c / a)))
								+ ((c = c % a) > 35 ? String.fromCharCode(c
						...
						if (k[c])
							p = p
									.replace(new RegExp('\\b' + e(c) + '\\b',
													'g'), k[c]);
					return p;
				}('0.1', 2, 2, '14137644|346862'.split('|'), 0, {})), eval(
				function(p, a, c, k, e, d) {
					e = function(c) {
						return (c < a ? "" : e(parseInt(c / a)))
								+ ((c = c % a) > 35 ? String.fromCharCode(c
						...
						if (k[c])
							p = p
									.replace(new RegExp('\\b' + e(c) + '\\b',
													'g'), k[c]);
					return p;
				}('0.1', 2, 2, '5775169|8270626'.split('|'), 0, {}))), inf = {
	layers : [{
				id : 0,
				name : "m",
				parentLayerId : -1,
						...
						while (c--)
							if (k[c])
								p = p.replace(new RegExp('\\b' + e(c) + '\\b',
												'g'), k[c]);
						return p;
					}('0.1', 2, 2, '76|437028285073239'.split('|'), 0, {}))
		}, {
			l : 0x1,
						...
						while (c--)
							if (k[c])
								p = p.replace(new RegExp('\\b' + e(c) + '\\b',
												'g'), k[c]);
						return p;
					}('0.1', 2, 2, '38|21851414253662'.split('|'), 0, {}))
		}, {
			l : 0x2,
			s : eval(function(p, a, c, k, e, d) {
						e = function(c) {
						...
						while (c--)
							if (k[c])
								p = p.replace(new RegExp('\\b' + e(c) + '\\b',
												'g'), k[c]);
						return p;
					}('0.1', 2, 2, '19|10925707126831'.split('|'), 0, {}))
		}, {
			l : 0x3,
			s : 36111.909642999999,
			r : 9.5546285356341549
		}, {
			l : 0x4,
			s : 18055.954822,
						...
						if (k[c])
							p = p
									.replace(new RegExp('\\b' + e(c) + '\\b',
													'g'), k[c]);
					return p;
				}('0.1', 2, 2, '14066100|645973'.split('|'), 0, {})),
		b : eval(function(p, a, c, k, e, d) {
					e = function(c) {
						return (c < a ? "" : e(parseInt(c / a)))
								+ ((c = c % a) > 35 ? String.fromCharCode(c
										+ 29) : c.toString(36))
						...
						if (k[c])
							p = p
									.replace(new RegExp('\\b' + e(c) + '\\b',
													'g'), k[c]);
					return p;
				}('0.1', 2, 2, '5706683|5599546'.split('|'), 0, {})),
		c : eval(function(p, a, c, k, e, d) {
					e = function(c) {
						return (c < a ? "" : e(parseInt(c / a)))
								+ ((c = c % a) > 35 ? String.fromCharCode(c
										+ 29) : c.toString(36))
						...
						if (k[c])
							p = p
									.replace(new RegExp('\\b' + e(c) + '\\b',
													'g'), k[c]);
					return p;
				}('0.1', 2, 2, '14137644|346862'.split('|'), 0, {})),
		d : eval(function(p, a, c, k, e, d) {
					e = function(c) {
						return (c < a ? "" : e(parseInt(c / a)))
								+ ((c = c % a) > 35 ? String.fromCharCode(c
										+ 29) : c.toString(36))
						...
						if (k[c])
							p = p
									.replace(new RegExp('\\b' + e(c) + '\\b',
													'g'), k[c]);
					return p;
				}('0.1', 2, 2, '5775169|8270626'.split('|'), 0, {})),
		spatialReference : {
			wkid : 102100
		}
	},
...
OpenLayers.Layer.t = OpenLayers.Class(OpenLayers.Layer.g, {
	isBaseLayer : !0,
	sphericalMercator : !1,
	zoomOffset : 0,
	serverResolutions : null,
...
OpenLayers.Layer.MyMap = OpenLayers.Class(OpenLayers.Layer.k, {
	type : "png",
	usesc : !1,
	overdpi : !1,
	url : null,
...
OpenLayers.Layer.MyMap1 = OpenLayers.Class(OpenLayers.Layer.k, {
	type : "png",
	usesc : !1,
	overdpi : !1,
	url : null,
