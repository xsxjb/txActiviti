...
AmCharts.AmChart = AmCharts.Class({
	...
	write: function(a) {
		...
		case "right":
			...
			break;
		case "left":
			h = document.createElement("div"), i = h.style, i.position = "relative", i.width = a.style.width, i.height = a.style.height, a.appendChild(h), f.position = "absolute", g.position = "relative", h.appendChild(b), h.appendChild(d)
...
AmCharts.Cuboid = AmCharts.Class({
	...
	draw: function() {
		...
		if (0 < f || 0 < g) {
			...
			var y = AmCharts.polygon(b, [0, 0, f, f, 0], [0, e, e + g, g, 0], q, i, 0, 0, 0, m),
				x = AmCharts.polygon(b, [d, d, d + f, d + f, d], [0, e, e + g, g, 0], q, i, 0, 0, 0, m);
			...
			if (0 < l) var D = AmCharts.line(b, [0, f, d + f], [e, e + g, e + g], k, l, j)
		}
		1 > Math.abs(e) && (e = 0);
		1 > Math.abs(d) && (d = 0);
		...
		e = 0 > e ? [s, t, u, w, y, x, v, n, D, b] : [n, D, u, w, y, x, s, t, v, b];
...
AmCharts.AmLegend = AmCharts.Class({
	...
	arrangeEntries: function() {
		...
		for (var y = w = 0, x = g, t = 0; t < s.length; t++) {
			var v = s[t];
			this.reversedOrder && (v = s[s.length - t - 1]);
			var u = v.getBBox(),
				D;
			this.equalWidths ? D = g + y * (r + l + this.markerLabelGap) : (D = x, x = x + u.width + g + l);
			D + u.width > m && 0 < t && (w++, y = 0, D = g, x = D + u.width + g + l);
			v.translate(D, (p + k) * w);
			y++;
			!isNaN(j) && y >= j && (y = 0, w++);
			q.push(v)
	...
	draw: function() {
		...
		if (!isNaN(a)) {
			...
			if (this.show) {
				...
				"H" != s ? (x = a - n / 2, d = b < i + l + 10 && "down" != s ? b + m : b - l - m) : (2 * m > l && (m = l / 2), d = b - l / 2, x = a < h + (j - h) / 2 ? a + m : a - n - m);
				d + l >= k && (d = k - l);
				d < i && (d = i);
				x < h && (x = h);
				x + n > j && (x = j - n);
				0 < t || 0 == m ? (p = AmCharts.rect(f, n, l, r, this.fillAlpha, this.borderThickness, p, this.borderAlpha, this.cornerRadius), this.showBullet && (f = AmCharts.circle(f, this.bulletSize, r, this.fillAlpha), f.translate(a, b), this.pointer = f)) : (k = [], t = [], "H" != s ? (h = a - x, h > n - m && (h = n - m), h < m && (h = m), k = [0, h - m, a - x, h + m, n, n, 0, 0], t = b < i + l + 10 && "down" != s ? [0, 0, b - d, 0, 0, l, l, 0] : [l, l, b - d, l, l, 0, 0, l]) : (i = b - d, i > l - m && (i = l - m), i < m && (i = m), t = [0, i - m, b - d, i + m, l, l, 0, 0], k = a < h + (j - h) / 2 ? [0, 0, a - x, 0, 0, n, n, 0] : [n, n, a - x, n, n, 0, 0, n]), p = AmCharts.polygon(f, k, t, r, this.fillAlpha, this.borderThickness, p, this.borderAlpha));
				g.push(p);
				p.toFront();
				y && y.toFront();
				e.toFront();
		...
		if (this.logarithmic && (0 >= this.getMin(0, this.data.length - 1) || 0 >= this.minimum)) this.fire("logarithmicAxisFailed", {
			...
			if (!isNaN(this.min) && !isNaN(this.max) && this.foundGraphs && Infinity != this.min && -Infinity != this.max) {
				...
				if (0 < u) {
					...
					for (e = this.fillAlpha = 0; e < u; e++) {
						var y = t[e],
							x = NaN;
						if (!isNaN(y.toValue)) {
							var x = this.getCoordinate(y.toValue),
								v = new this.axisItemRenderer(this, x, "", !0, NaN, NaN, y);
							this.pushAxisItem(v)
						}
						var D = NaN;
						isNaN(y.value) || (D = this.getCoordinate(y.value), v = new this.axisItemRenderer(this, D, y.label, !0, NaN, (x - D) / 2, y), this.pushAxisItem(v));
						isNaN(x - D) || (v = new this.guideFillRenderer(this, D, x, y), this.pushAxisItem(v), v = v.graphics(), y.graphics = v, y.balloonText && this.addEventListeners(v, y))
					}
					this.fillAlpha = w
				}
				t = !1;
				for (e = p; e < q; e += r) v = AmCharts.roundTo(this.step * e + this.min, d), -1 != String(v).indexOf("e") && (t = !0, String(v).split("e"));
				...
				for (e = p; e < q; e += r) if (p = this.step * e + this.min, p = AmCharts.roundTo(p, this.maxDecCount + 1), !(this.integersOnly && Math.round(p) != p)) {
					...
					t ? (v = -1 == String(p).indexOf("e") ? p.toExponential(15) : String(p), v = v.split("e"), d = Number(v[0]), v = Number(v[1]), d = AmCharts.roundTo(d, 14), 10 == d && (d = 1, v += 1), v = d + "e" + v, 0 == p && (v = "0"), 1 == p && (v = "1")) : (i && (d = String(p).split("."), s.precision = d[1] ? d[1].length : -1), v = this.usePrefixes ? AmCharts.addPrefix(p, a.prefixesOfBigNumbers, a.prefixesOfSmallNumbers, s) : AmCharts.formatNumber(p, s, s.precision));
					...
					if (p == this.baseValue && "radar" != j) {
						var K, F, u = this.viW,
							...
							p = this.viX,
							v = this.viY;
						"H" == this.orientation ? 0 <= d && d <= u + 1 && (K = [d, d, d + f], F = [w, 0, g]) : 0 <= d && d <= w + 1 && (K = [0, u, u + f], F = [d, d, d + g]);
						K && (d = AmCharts.fitToBounds(2 * this.gridAlpha, 0, 1), d = AmCharts.line(a.container, K, F, this.gridColor, d, 1, this.dashLength), d.translate(p, v), this.grid0 = d, a.axesSet.push(d), d.toBack())
					}
				}
				e = this.baseValue;
	...
	stackGraphs: function(a) {
		...
		for (m = this.start; m <= this.end; m++) {
			...
			for (l = 0; l < i.length; l++) if (k = i[l], !k.hidden && (j = k.type, k.chart == this.chart && (k.valueAxis == this && a == j && k.stackable) && (j = this.data[m].axes[this.id].graphs[k.id], h = j.values.value, !isNaN(h)))) {
				...
				"regular" == b && (linetype && (isNaN(d[m]) ? (d[m] = h, j.values.close = h, j.values.open = this.baseValue) : (j.values.close = isNaN(h) ? d[m] : h + d[m], j.values.open = d[m], d[m] = j.values.close)), "column" == a && !isNaN(h) && (j.values.close = h, 0 > h ? (j.values.close = h, isNaN(e[m]) ? j.values.open = r : (j.values.close += e[m], j.values.open = e[m]), e[m] = j.values.close) : (j.values.close = h, isNaN(f[m]) ? j.values.open = r : (j.values.close += f[m], j.values.open = f[m]), f[m] = j.values.close)))
			}
		}
...
AmCharts.CategoryAxis = AmCharts.Class({
	...
	draw: function() {
		...
		if (AmCharts.ifArray(a)) {
			...
			if (this.parseDates && !this.equalSpacing) {
				...
				if (this.parseDates && this.equalSpacing) {
					...
					l = AmCharts.resetDateToMin(new Date(this.startTime - r), e, m, s).getTime();
				...
				for (h = 0; g <= this.end + 2; g += t) {
					...
					s = this.cellWidth;
					m && (s = NaN);
					k = new this.axisItemRenderer(this, k, l, !0, s, p, void 0, !1, p);
					this.pushAxisItem(k)
				}
			}
		...
		for (g = 0; g < i; g++) j = this.guides[g], h = t = d = NaN, j.toCategory && (h = b.getCategoryIndexByValue(j.toCategory), isNaN(h) || (d = this.getCoordinate(h - f), k = new this.axisItemRenderer(this, d, "", !0, NaN, NaN, j), this.pushAxisItem(k))), j.category && (h = b.getCategoryIndexByValue(j.category), isNaN(h) || (t = this.getCoordinate(h - f), h = (d - t) / 2, k = new this.axisItemRenderer(this, t, j.label, !0, NaN, h, j), this.pushAxisItem(k))), j.toDate && (this.equalSpacing ? (h = b.getClosestIndex(this.data, "time", j.toDate.getTime(), !1, 0, this.data.length - 1), isNaN(h) || (d = this.getCoordinate(h - f))) : d = (j.toDate.getTime() - this.startTime) * this.stepWidth, k = new this.axisItemRenderer(this, d, "", !0, NaN, NaN, j), this.pushAxisItem(k)), j.date && (this.equalSpacing ? (h = b.getClosestIndex(this.data, "time", j.date.getTime(), !1, 0, this.data.length - 1), isNaN(h) || (t = this.getCoordinate(h - f))) : t = (j.date.getTime() - this.startTime) * this.stepWidth, h = (d - t) / 2, k = "H" == this.orientation ? new this.axisItemRenderer(this, t, j.label, !1, 2 * h, NaN, j) : new this.axisItemRenderer(this, t, j.label, !1, NaN, h, j), this.pushAxisItem(k)), d = new this.guideFillRenderer(this, t, d, j), t = d.graphics(), this.pushAxisItem(d), j.graphics = t, t.index = g, j.balloonText && this.addEventListeners(t, j);
		this.axisCreated = !0;
		b = this.x;
		f = this.y;
...
AmCharts.RecItem = AmCharts.Class({
	construct: function(a, b, d, e, f, g, h, i, j) {
		...
		var q = a.axisThickness,
			...
			ca = a.offset,
			ka, U;
		h ? (K = !0, isNaN(h.tickLength) || (t = h.tickLength), void 0 != h.lineColor && (y = h.lineColor), isNaN(h.lineAlpha) || (u = h.lineAlpha), isNaN(h.dashLength) || (x = h.dashLength), isNaN(h.lineThickness) || (w = h.lineThickness), !0 == h.inside && (N = !0), isNaN(h.labelRotation) || (F = h.labelRotation)) : "" == d && (t = 0);
		U = "start";
		f && (U = "middle");
		var Q = F * Math.PI / 180,
			Z, E = 0,
			...
			fa = Z = 0;
		"V" == V && (F = 0);
		if (K) var W = AmCharts.text(r, d, m, k, l, U, i),
			fa = W.getBBox().width;
		if ("H" == V) {
			if (0 <= b && b <= ba + 1 && (0 < t && (0 < s && b + j <= ba + 1) && (ka = AmCharts.line(r, [b + j, b + j], [0, t], n, s, w), p.push(ka)), 0 < u && (U = AmCharts.line(r, [b, b + P, b + P], [ea, ea + G, G], y, u, w, x), p.push(U))), C = 0, E = b, h && 90 == F && (E -= l), !1 == e ? (U = "start", C = "bottom" == O ? N ? C + t : C - t : N ? C - t : C + t, E += 3, f && (E += f / 2, U = "middle"), 0 < F && (U = "middle")) : U = "middle", 1 == Y && (0 < D && !h && T < ba) && (e = AmCharts.fitToBounds(b, 0, ba), T = AmCharts.fitToBounds(T, 0, ba), Z = e - T, 0 < Z && (fill = AmCharts.rect(r, Z, a.height, v, D), fill.translate(e - Z + P, G), p.push(fill))), "bottom" == O ? (C += ea + l / 2 + ca, N ? 0 < F ? (C = ea - fa / 2 * Math.sin(Q) - t - 3, E += fa / 2 * Math.cos(Q)) : C -= t + l + 3 + 3 : 0 < F ? (C = ea + fa / 2 * Math.sin(Q) + t + 3, E -= fa / 2 * Math.cos(Q)) : C += t + q + 3 + 3) : (C += G + l / 2 - ca, E += P, N ? 0 < F ? (C = fa / 2 * Math.sin(Q) + t + 3, E -= fa / 2 * Math.cos(Q)) : C += t + 3 : 0 < F ? (C = -(fa / 2) * Math.sin(Q) - t - 6, E += fa / 2 * Math.cos(Q)) : C -= t + l + 3 + q + 3), "bottom" == O ? Z = (N ? ea - t - 1 : ea + q - 1) + ca : (z = P, Z = (N ? G : G - t - q + 1) - ca), g && (E += g), G = E, 0 < F && (G += fa / 2 * Math.cos(Q)), W && (O = 0, N && (O = fa * Math.cos(Q)), G + O > ba + 1 || 0 > G)) W.remove(), W = null
		} else {
			0 <= b && b <= ea + 1 && (0 < t && (0 < s && b + j <= ea + 1) && (ka = AmCharts.line(r, [0, t], [b + j, b + j], n, s, w), p.push(ka)), 0 < u && (U = AmCharts.line(r, [0, P, ba + P], [b, b + G, b + G], y, u, w, x), p.push(U)));
			U = "end";
			if (!0 == N && "left" == O || !1 == N && "right" == O) U = "start";
			C = b - l / 2;
			1 == Y && (0 < D && !h) && (e = AmCharts.fitToBounds(b, 0, ea), T = AmCharts.fitToBounds(T, 0, ea), Q = e - T, fill = AmCharts.polygon(r, [0, a.width, a.width, 0], [0, 0, Q, Q], v, D), fill.translate(P, e - Q + G), p.push(fill));
			C += l / 2;
			"right" == O ? (E += P + ba + ca, C += G, N ? (E -= t + 4, g || (C -= l / 2 + 3)) : (E += t + 4 + q, C -= 2)) : N ? (E += t + 4 - ca, g || (C -= l / 2 + 3), h && (E += P, C += G)) : (E += -t - q - 4 - 2 - ca, C -= 2);
			ka && ("right" == O ? (z += P + ca + ba, Z += G, z = N ? z - q : z + q) : (z -= ca, N || (z -= t + q)));
			g && (C += g);
			N = -3;
			"right" == O && (N += G);
			if (W && (C > ea + 1 || C < N)) W.remove(), W = null
		}
		ka && ka.translate(z, Z);
		!1 == a.visible && (ka && ka.remove(), W && (W.remove(), W = null));
		W && (W.attr({
			"text-anchor": U
		}), W.translate(E, C), 0 != F && W.rotate(-F), a.allLabels.push(W), " " != d && (this.label = W));
		a.counter = 0 == Y ? 1 : 0;
		a.previousCoord = b;
...
AmCharts.RadItem = AmCharts.Class({
	construct: function(a, b, d, e, f, g, h) {
		...
		var m = a.axisColor,
			...
			D, g = a.x,
			K = a.y;
		h ? (y = !0, isNaN(h.tickLength) || (p = h.tickLength), void 0 != h.lineColor && (s = h.lineColor), isNaN(h.lineAlpha) || (q = h.lineAlpha), isNaN(h.dashLength) || (t = h.dashLength), isNaN(h.lineThickness) || (n = h.lineThickness), !0 == h.inside && (x = !0)) : d || (q /= 3, p /= 2);
		var F = "end",
			Y = -1;
		x && (F = "start", Y = 1);
		if (y) {
			...
			e.push(N);
			this.label = N;
			D = AmCharts.line(l, [g, g + p * Y], [b, b], m, r, n);
			e.push(D)
		}
		b = a.y - b;
		if ("polygons" == v) {
			for (var P = [], G = [], V = a.data.length, d = 0; d < V; d++) i = 180 - 360 / V * d, P.push(b * Math.sin(i / 180 * Math.PI)), G.push(b * Math.cos(i / 180 * Math.PI));
			P.push(P[0]);
			G.push(G[0]);
			d = AmCharts.line(l, P, G, s, q, n, t)
		...
		if (1 == f && 0 < w && !h) {
			h = a.previousCoord;
			if ("polygons" == v) {
				for (d = V; 0 <= d; d--) i = 180 - 360 / V * d, P.push(h * Math.sin(i / 180 * Math.PI)), G.push(h * Math.cos(i / 180 * Math.PI));
				P = AmCharts.polygon(l, P, G, u, w)
...
AmCharts.AmGraph = AmCharts.Class({
	...
	createSerialGraph: function() {
		...
		if (!Y && ("line" == g || "smoothedLine" == g || "step" == g)) if (1 == d.length && ("step" != g && "none" == this.bullet) && (this.bullet = "round", this.resetBullet = !0), Da || void 0 != xa) {
			var ya = G;
			ya > f.max && (ya = f.max);
			ya < f.min && (ya = f.min);
		...
		if ("line" == g || "step" == g || "smoothedLine" == g) {
			if (0 < K) for (L = K - 1; - 1 < L; L--) if (C = d[L], z = C.axes[f.id].graphs[a], fa = z.values.value) {
				K = L;
				break
			}
			if (F < d.length - 1) for (L = F + 1; L < d.length; L++) if (C = d[L], z = C.axes[f.id].graphs[a], fa = z.values.value) {
				F = L;
				break
			}
		}
		...
		for (L = K; L <= F; L++) {
			...
			isNaN(z.alpha) || (Ha = z.alpha);
			var qa = z.values;
			f.recalculateToPercents && (qa = z.percents);
			if (qa) {
				...
				Y || (z.x = na); - 1E5 > na && (na = -1E5);
				na > i + 1E5 && (na = i + 1E5);
				l ? (B = A, R = M, M = A = na, isNaN(ia) && !this.fillToGraph && (R = P), Qa = za, Ra = Aa) : (R = B = na, isNaN(ia) && !this.fillToGraph && (M = P));
				switch (g) {
				case "line":
					isNaN(S) ? p || (this.drawLineGraph(q, n, ga, ha), q = [], n = [], ga = [], ha = []) : (z.isNegative = S < G ? !0 : !1, q.push(B), n.push(A), X = B, $ = A, oa = B, pa = A, Ga && (!isNaN(M) && !isNaN(R)) && (ga.push(R), ha.push(M)));
					break;
				case "smoothedLine":
					isNaN(S) ? p || (this.drawSmoothedGraph(q, n, ga, ha), q = [], n = [], ga = [], ha = []) : (z.isNegative = S < G ? !0 : !1, q.push(B), n.push(A), X = B, $ = A, oa = B, pa = A, Ga && (!isNaN(M) && !isNaN(R)) && (ga.push(R), ha.push(M)));
					break;
				case "step":
					isNaN(S) ? p || (t = NaN, this.drawLineGraph(q, n, ga, ha), q = [], n = [], ga = [], ha = []) : (z.isNegative = S < G ? !0 : !1, l ? (isNaN(s) || (q.push(s), n.push(A - va)), n.push(A - va), q.push(B), n.push(A + Ia), q.push(B), Ga && (!isNaN(M) && !isNaN(R)) && (ga.push(R), ha.push(M - va), ga.push(R), ha.push(M + Ia))) : (isNaN(t) || (n.push(t), q.push(B - va)), q.push(B - va), n.push(A), q.push(B + Ia), n.push(A), Ga && (!isNaN(M) && !isNaN(R)) && (ga.push(R - va), ha.push(M), ga.push(R + Ia), ha.push(M))), s = B, t = A, X = B, $ = A, oa = B, pa = A);
					break;
				case "column":
					var ma = la;
					void 0 != z.lineColor && (ma = z.lineColor);
					if (!isNaN(S)) {
						S < G ? (z.isNegative = !0, Da && (ua = Da), void 0 != xa && (la = xa)) : z.isNegative = !1;
						var nb = f.min,
							ob = f.max;
						if (!(S < nb && (ia < nb || void 0 == ia) || S > ob && ia > ob)) if (l) {
							if ("3d" == v) var I = A - 0.5 * (h + Q) + Q / 2 + x * V,
								J = R + y * V;
							else I = A - (m / 2 - V) * (h + Q) + Q / 2, J = R;
							var H = h,
								X = B,
								$ = I + h / 2,
								oa = B,
							...
							var aa = B - R,
								Bb = J,
								J = AmCharts.fitToBounds(J, 0, i),
								aa = aa + (Bb - J),
								aa = AmCharts.fitToBounds(aa, -J, i - J + y * V);
							if (I < j && 0 < H && (ja = new AmCharts.Cuboid(e, aa, H, y, x, ua, Ha, O, ma, T, U, r, l), "bottom" != D)) if (D = f.reversed ? "left" : "right", 0 > S) D = f.reversed ? "right" : "left";
							else if ("regular" == v || "100%" == v) X += this.dx
						} else {
							"3d" == v ? (J = B - 0.5 * (h + Q) + Q / 2 + y * V, I = M + x * V) : (J = B - (m / 2 - V) * (h + Q) + Q / 2, I = M);
							H = h;
							X = J + h / 2;
							$ = A;
							oa = J + h / 2;
							...
							var aa = A - M,
								...
								aa = aa + (Cb - I),
								aa = AmCharts.fitToBounds(aa, -I + x * V, j - I);
							if (J < i + V * y && 0 < H) if (ja = new AmCharts.Cuboid(e, H, aa, y, x, ua, Ha, O, ma, this.lineAlpha, U, r, l), 0 > S && "middle" != D) D = "bottom";
							else if (D = ka, "regular" == v || "100%" == v) $ += this.dy
						}
						if (ja) {
							var ra = ja.set;
							ra.translate(J, I);
							this.columnsSet.push(ra);
							z.url && ra.setAttr("cursor", "pointer");
							if (!Y) {
								"none" == v && (w = l ? (this.end + 1 - L) * u - b : u * L + b);
								"3d" == v && (l ? (w = (u - b) * (this.end + 1 - L), X += y * this.columnIndex, oa += y * this.columnIndex, z.y += y * this.columnIndex) : (w = (u - b) * (L + 1), X += 3, $ += x * this.columnIndex + 7, pa += x * this.columnIndex, z.y += x * this.columnIndex));
								if ("regular" == v || "100%" == v) D = "middle", w = l ? 0 < qa.value ? (this.end + 1 - L) * u + b : (this.end + 1 - L) * u - b : 0 < qa.value ? u * L + b : u * L - b;
								...
								this.animateColumns(ja, L, B, R, A, M);
								this.addListeners(ra, z)
							}
							z.columnSprite = ra
						}
					}
				...
				case "candlestick":
					if (!isNaN(ia) && !isNaN(Za) && !isNaN(Ya) && !isNaN(S)) {
						var Sa, $a;
						S < ia && (z.isNegative = !0, Da && (ua = Da), Fa && (Ha = Fa), void 0 != xa && (la = xa));
						ma = la;
						void 0 != z.lineColor && (ma = z.lineColor);
						if (l) {
							if (I = A - h / 2, J = R, H = h, I + H > j && (H = j - I), 0 > I && (H += I, I = 0), I < j && 0 < H) {
								var ab, bb;
								S > ia ? (ab = [B, Ra], bb = [R, Qa]) : (ab = [R, Ra], bb = [B, Qa]);
								A < j && 0 < A && (Sa = AmCharts.line(e, ab, [A, A], ma, T, O), $a = AmCharts.line(e, bb, [A, A], ma, T, O));
								aa = B - R;
								ja = new AmCharts.Cuboid(e, aa, H, y, x, ua, Ea, O, ma, T, U, r, l)
							}
						} else if (J = B - h / 2, I = M + O / 2, H = h, J + H > i && (H = i - J), 0 > J && (H += J, J = 0), aa = A - M, J < i && 0 < H) {
							var ja = new AmCharts.Cuboid(e, H, aa, y, x, ua, Ha, O, ma, T, U, r, l),
								cb, db;
							S > ia ? (cb = [A, Aa], db = [M, za]) : (cb = [M, Aa], db = [A, za]);
							B < i && 0 < B && (Sa = AmCharts.line(e, [B, B], cb, ma, T, O), $a = AmCharts.line(e, [B, B], db, ma, T, O))
						}
						ja && (ra = ja.set, ca.push(ra), ra.translate(J, I), z.url && ra.setAttr("cursor", "pointer"), Sa && (ca.push(Sa), ca.push($a)), X = B, $ = A, oa = B, pa = A, Y || (z.x = l ? I + H / 2 : J + H / 2, this.animateColumns(ja, L, B, R, A, M), this.addListeners(ra, z)))
					}
					break;
				case "ohlc":
					if (!isNaN(ia) && !isNaN(Za) && !isNaN(Ya) && !isNaN(S)) {
						S < ia && (z.isNegative = !0, void 0 != xa && (la = xa));
						var eb, fb, gb;
						if (l) {
							var hb = A - h / 2,
								hb = AmCharts.fitToBounds(hb, 0, j),
								pb = AmCharts.fitToBounds(A, 0, j),
								ib = A + h / 2,
								ib = AmCharts.fitToBounds(ib, 0, j);
							fb = AmCharts.line(e, [R, R], [hb, pb], la, T, O, ba);
							0 < A && A < j && (eb = AmCharts.line(e, [Qa, Ra], [A, A], la, T, O, ba));
							gb = AmCharts.line(e, [B, B], [pb, ib], la, T, O, ba)
						} else {
							var jb = B - h / 2,
								jb = AmCharts.fitToBounds(jb, 0, i),
								qb = AmCharts.fitToBounds(B, 0, i),
								kb = B + h / 2,
								kb = AmCharts.fitToBounds(kb, 0, i);
							fb = AmCharts.line(e, [jb, qb], [M, M], la, T, O, ba);
							0 < B && B < i && (eb = AmCharts.line(e, [B, B], [za, Aa], la, T, O, ba));
							gb = AmCharts.line(e, [qb, kb], [A, A], la, T, O, ba)
						}
						ca.push(fb);
						ca.push(eb);
						ca.push(gb);
				...
				if (!Y && !isNaN(S)) {
					...
					if (this.end - this.start <= rb || 0 == rb) {
						var Ba = this.createBullet(z, oa, pa, L);
						Ba || (Ba = 0);
						var sb = this.labelText;
						if (sb) {
							var da = this.createLabel(z, 0, 0, sb),
								wa = 0,
								Ca = 0,
								tb = da.getBBox(),
								Ta = tb.width,
							...
							case "bottom":
								...
								break;
							case "middle":
								"column" == g && (l ? (wa = -(B - R) / 2 - y, 0 > aa && (wa += y), Math.abs(B - R) < Ta && !this.showAllValueLabels && (da.remove(), da = null)) : (Ca = -(A - M) / 2, 0 > aa && (Ca -= x), Math.abs(A - M) < lb && !this.showAllValueLabels && (da.remove(), da = null)))
							}
						...
						if ("column" == g && "regular" == v || "100%" == v) {
							var vb = f.totalText;
							if (vb) {
								var Ja = this.createLabel(z, 0, 0, vb),
									wb = Ja.getBBox(),
									xb = wb.width,
									yb = wb.height,
									Ka, La, zb = f.totals[L];
								zb && zb.remove();
								l ? (La = A, Ka = 0 > S ? B - xb / 2 - 2 : B + xb / 2 + 3) : (Ka = B, La = 0 > S ? A + yb / 2 : A - yb / 2 - 3);
								Ja.translate(Ka, La);
								f.totals[L] = Ja;
	...
	drawLineGraph: function(a, b, d, e) {
		if (1 < a.length) {
			...
			if (0 < f && (j = a.join(";").split(";"), k = b.join(";").split(";"), "serial" == this.chartType && (0 < d.length ? (d.reverse(), e.reverse(), j = a.concat(d), k = b.concat(e)) : this.rotate ? (k.push(k[k.length - 1]), j.push(n), k.push(k[0]), j.push(n), k.push(k[0]), j.push(j[0])) : (j.push(j[j.length - 1]), k.push(n), j.push(j[0]), k.push(n), j.push(a[0]), k.push(k[0]))), a = AmCharts.polygon(g, j, k, m, f, 0, 0, 0, this.gradientRotation), h.push(a), p || void 0 != r)) q || (q = f), p || (p = r), g = AmCharts.polygon(g, j, k, p, q, 0, 0, 0, this.gradientRotation), i.push(g);
			this.applyMask(i, h)
		}
	},
	...
	drawSmoothedGraph: function(a, b, d, e) {
		if (1 < a.length) {
			...
			h.push(s.path);
			void 0 != r && (j = new AmCharts.Bezier(g, a, b, r, j, k, m, 0, f), i.push(j.path));
			if (0 < l && (k = a.join(";").split(";"), s = b.join(";").split(";"), j = "", 0 < d.length ? (d.reverse(), e.reverse(), k = a.concat(d), s = b.concat(e)) : (this.rotate ? (j += " L" + n + "," + b[b.length - 1], j += " L" + n + "," + b[0]) : (j += " L" + a[a.length - 1] + "," + n, j += " L" + a[0] + "," + n), j += " L" + a[0] + "," + b[0]), d = new AmCharts.Bezier(g, k, s, NaN, 0, 0, m, l, f, j), h.push(d.path), p || void 0 != r)) q || (q = l), p || (p = r), a = new AmCharts.Bezier(g, a, b, NaN, 0, 0, p, q, f, j), i.push(a.path);
			this.applyMask(i, h)
		}
	},
...
AmCharts.ChartCursor = AmCharts.Class({
	...
	createCursor: function() {
		...
		this.set.push(b);
		a ? (f && (h.pointerWidth = 0), "right" == e ? f ? h.setBounds(m, r + l, m + p + k, r + q + l) : h.setBounds(m + p + k + g, r + l, m + p + 1E3, r + q + l) : f ? h.setBounds(m, r, p + m, q + r) : h.setBounds(-1E3, -1E3, m - n - g, r + q + 15)) : (h.maxWidth = p, d.parseDates && (n = 0, h.pointerWidth = 0), "top" == e ? f ? h.setBounds(m + k, r + l, p + k + m, q + r) : h.setBounds(m + k, -1E3, p + k + m, r + l - n - g) : f ? h.setBounds(m, r, p + m, q + r - n) : h.setBounds(m, r + q + n + g - 1, m + p, r + q + n + g));
		this.hideCursor()
	},
		...
		if (this.enabled) {
			...
			if (d.mouseIsOver || v || D || this.forceShow) if (this.forceShow = !1, D) {
				...
				0 != e && (k = {
					type: "zoomed",
					target: this
				}, k.chart = this.chart, g.parseDates && !g.equalSpacing ? (d + e > w && (e = w - d), v + e < u && (e = u - v), k.start = v + e, k.end = d + e, this.fire(k.type, k)) : h + e >= this.data.length || 0 > j + e || (k.start = j + e, k.end = h + e, this.fire(k.type, k)))
			} else {
				...
				v && (q ? this.updateSelectionSize(NaN, p) : this.updateSelectionSize(p, NaN));
				u = !0;
				v && (u = !1);
				this.categoryBalloonEnabled && u ? (q ? (n && ("right" == x ? t.setBounds(j, k + i, j + l + h, k + p + i) : t.setBounds(j, k + i, j + l + h, k + p)), "right" == x ? n ? t.setPosition(j + l + h, k + p + i) : t.setPosition(j + l + h + F, k + p + i) : n ? t.setPosition(j, k + p) : t.setPosition(j - F, k + p)) : "top" == x ? n ? t.setPosition(j + p + h, k + i) : t.setPosition(j + p + h, k + i - F + 1) : n ? t.setPosition(j + p, k + m) : t.setPosition(j + p, k + m + F - 1), g.parseDates ? (g = AmCharts.formatDate(r.category, this.categoryBalloonDateFormat), -1 != g.indexOf("fff") && (g = AmCharts.formatMilliseconds(g, r.category)), t.showBalloon(g)) : t.showBalloon(r.category)) : t.hide();
				K && this.bulletsEnabled && this.showBullets();
				this.destroyValueBalloons();
				if (K && this.valueBalloonsEnabled && u && d.balloon.enabled) {
					this.valueBalloons = g = [];
					if (this.oneBalloonOnly) for (var i = Infinity, Y, u = 0; u < K.length; u++) s = K[u], s.showBalloon && (!s.hidden && s.balloonText) && (t = r.axes[s.valueAxis.id].graphs[s.id], w = t.y, isNaN(w) || (q ? Math.abs(e - w) < i && (i = Math.abs(e - w), Y = s) : Math.abs(f - w) < i && (i = Math.abs(f - w), Y = s)));
					for (u = 0; u < K.length; u++) if (s = K[u], !(this.oneBalloonOnly && s != Y) && (s.showBalloon && !s.hidden && s.balloonText) && (t = r.axes[s.valueAxis.id].graphs[s.id], w = t.y, !isNaN(w))) {
						p = t.x;
						n = !0;
						if (q) {
							if (i = w, 0 > p || p > m) n = !1
						} else if (i = p, p = w, 0 > i || i > l + h) n = !1;
						n && (n = s.valueBalloon, x = d.getBalloonColor(s, t), n.setBounds(j, k, j + l, k + m), n.pointerOrientation = "H", n.changeColor(x), void 0 != s.balloonAlpha && (n.fillAlpha = s.balloonAlpha), void 0 != s.balloonTextColor && (n.color = s.balloonTextColor), n.setPosition(i + j, p + k), s = d.formatString(s.balloonText, t, s), "" != s && n.showBalloon(s), !q && n.set && n.set.hide(), g.push({
							yy: w,
							balloon: n
						}))
					}
...
AmCharts.wedge = function(a, b, d, e, f, g, h, i, j, k, l) {
	...
	var e = a.set(),
		K;
	p && (q = m(10 * q), t = m(10 * t), w = m(10 * w), x = m(10 * x), s = m(10 * s), u = m(10 * u), y = m(10 * y), n = m(10 * n), b = m(10 * b), j = m(10 * j), d = m(10 * d), g *= 10, h *= 10, i *= 10, r *= 10, 1 > Math.abs(f) && (1 >= Math.abs(w - t) && 1 >= Math.abs(y - u)) && (K = !0));
	f = "";
	if (0 < j) {
		p ? (path = " M" + q + "," + (s + j) + " L" + t + "," + (u + j), K || (path += " A" + (b - g) + "," + (j + d - h) + "," + (b + g) + "," + (j + d + h) + "," + t + "," + (u + j) + "," + w + "," + (y + j)), path += " L" + x + "," + (n + j), 0 < i && (K || (path += " B" + (b - i) + "," + (j + d - r) + "," + (b + i) + "," + (j + d + r) + "," + x + "," + (j + n) + "," + q + "," + (j + s)))) : (path = " M" + q + "," + (s + j) + " L" + t + "," + (u + j), path += " A" + g + "," + h + ",0," + D + ",1," + w + "," + (y + j) + " L" + x + "," + (n + j), 0 < i && (path += " A" + i + "," + r + ",0," + D + ",0," + q + "," + (s + j)));
		path += " Z";
		c = a.path(path, void 0, void 0, "1000,1000").attr(v);
		e.push(c);
		var F = a.path(" M" + q + "," + s + " L" + q + "," + (s + j) + " L" + t + "," + (u + j) + " L" + t + "," + u + " L" + q + "," + s + " Z", void 0, void 0, "1000,1000").attr(v),
			j = a.path(" M" + w + "," + y + " L" + w + "," + (y + j) + " L" + x + "," + (n + j) + " L" + x + "," + n + " L" + w + "," + y + " Z", void 0, void 0, "1000,1000").attr(v);
		e.push(F);
		e.push(j)
	}
	p ? (K || (f = " A" + m(b - g) + "," + m(d - h) + "," + m(b + g) + "," + m(d + h) + "," + m(t) + "," + m(u) + "," + m(w) + "," + m(y)), g = " M" + m(q) + "," + m(s) + " L" + m(t) + "," + m(u) + f + " L" + m(x) + "," + m(n)) : g = " M" + q + "," + s + " L" + t + "," + u + (" A" + g + "," + h + ",0," + D + ",1," + w + "," + y) + " L" + x + "," + n;
	0 < i && (p ? K || (g += " B" + (b - i) + "," + (d - r) + "," + (b + i) + "," + (d + r) + "," + x + "," + n + "," + q + "," + s) : g += " A" + i + "," + r + ",0," + D + ",0," + q + "," + s);
		...
		if (AmCharts.ifArray(b)) {
			if (0 < a.realWidth && 0 < a.realHeight) {
				...
				for (l = 0; l < b.length; l++) if (f = b[l], !0 != f.hidden && 0 < f.percents) {
					...
					i = AmCharts.wedge(e, m, r, g, s, p, k, h, a.depth3D, i, a.gradientRatio);
					...
					if (a.labelsEnabled && a.labelText && f.percents >= a.hideLabelsPercent) {
						...
						if (0 <= q) {
							var w;
							90 >= s && 0 <= s ? (w = 0, u = "start", n = 8) : 360 >= s && 270 < s ? (w = 1, u = "start", n = 8) : 270 >= s && 180 < s ? (w = 2, u = "end", n = -8) : 180 >= s && 90 < s && (w = 3, u = "end", n = -8);
							f.labelQuarter = w
						} else u = "middle";
