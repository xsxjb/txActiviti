...
function CyclePump( id,x,y ){
	...
	this.win = "/XMCP/genericPlcControlStationStatusAction!queryCyclePump.action";
	...
	this.init = function( json ){
		...
		if( this.direction == "horizontal" ){
			this.bark.src = "../../../js/icons/xhb1-h-0.png";
		}else{
			this.bark.src = "../../../js/icons/xhb1-v-0.png";
		}
	}
		...
		try {
			...
			if( parseInt(this.json[heatUnitId+'-'+this.deviceId+'-circPumpState']) == 0 ){
				...
				if( parseInt(this.json[heatUnitId+'-'+this.deviceId+'-circPumpFrequency']) >= 5.0 ){
					if(  parseInt(this.json[heatUnitId+'-'+this.deviceId+'-circPumpFault']) >= 1 ){
						this.status = '故障2';
					}else{
						this.status = '本地运行';
					}
			...
			}else if( parseInt(this.json[heatUnitId+'-'+this.deviceId+'-circPumpState']) >= 1  ){
	...
	this.clickView = function( mouseX,mouseY ){
		...
		if( this.isPop == 1 && this.model == 'control' ){
			...
			if( this.isError == 0 ){
				...
				$("#controlPage").css({top:(canvas.height-380)/2+50 ,left:(canvas.width-570)/2});
				$("#controlPage").css({width:570 ,height:380});
	...
	this.clickAdd = function(){
		...
		this.name = $('#deviceId option:selected').text();
	...
	this.animation = function(){
		...
		if( this.status == '故障1' ){
			if( this.redblack == 1 ){
				this.bark.src = "../../../js/icons/xhb1-"+dir+"-2.png";//黄色
				this.redblack = 0;
			}else{
				this.bark.src = "../../../js/icons/xhb1-"+dir+"-0.png";//红色
				this.redblack = 1;
			}
		}else if( this.status == '故障2' ){
			if( this.redblack == 1 ){
				this.bark.src = "../../../js/icons/xhb1-"+dir+"-2.png";//黄色
				this.redblack = 0;
			}else{
				this.bark.src = "../../../js/icons/xhb1-"+dir+"-1.png";//红色
				this.redblack = 1;
			}
		}else if( this.status == '运行'  ){
			this.bark.src = "../../../js/icons/xhb1-"+dir+"-1.png";//红色
		}else if( this.status == '本地运行'  ){
			this.bark.src = "../../../js/icons/xhb1-"+dir+"-1Y.png";//红色
		}else if( this.status == '未运行'  ){
			this.bark.src = "../../../js/icons/xhb1-"+dir+"-0.png";//绿色
		}
