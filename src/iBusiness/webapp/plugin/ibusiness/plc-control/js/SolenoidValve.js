 ...
 *	@time	2013-05-20
 */
function SolenoidValve( id,x,y ){

	this.model = 'design';//设定2个模式（design/control）
	this.id = id;
	this.name = '泄压阀';
	this.type = 'SolenoidValve';
	this.title = '标题';
	...
	this.win = "/XMCP/genericPlcControlStationStatusAction!querySolenoidValve.action";
	...
	this.updateStatus = function( json ){
        ...
        try {
			...
			if( parseInt(this.json[heatUnitId+'-'+this.deviceId+'-supplyPumpState']) == 0 ){
				this.status = '未运行';
			}else if( parseInt(this.json[heatUnitId+'-'+this.deviceId+'-supplyPumpState']) >= 1 && parseInt(this.json[heatUnitId+'-'+this.deviceId+'-supplyPumpFault']) == 0 ){
				this.status = '运行';
			}else if( parseInt(this.json[heatUnitId+'-'+this.deviceId+'-supplyPumpState']) >= 1 && parseInt(this.json[heatUnitId+'-'+this.deviceId+'-supplyPumpFault']) >= 1 ){
				this.status = '故障';
			}
	　　} catch(err) {
			this.status = '未运行';
