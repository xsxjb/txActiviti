 ...
 *	@time	2013-05-29
 */
function SupplyPump( id,x,y ){

	this.model = 'design';//设定2个模式（design/control）
	this.id = id;
	this.name = '补水泵';
	this.type = 'SupplyPump';
	...
	this.win = "/XMCP/genericPlcControlStationStatusAction!querySupplyPump.action";
	...
	this.updateStatus = function( json ){
		...
		try {
			...
			var circPumpID = heatStationId+''+heatUnitId+''+this.deviceId;
			...
			var supplyPumpFault = this.json[heatUnitId+'-'+this.deviceId+'-supplyPumpFault'];
			//console.log( "this.json.supplyPumpState-->"+ (parseInt(this.json.supplyPumpState) == 1) );
			if( parseInt(this.json[heatUnitId+'-'+this.deviceId+'-supplyPumpState']) == 0 ){
				//console.log( "this.json.supplyPumpFrequency-->"+ parseInt(this.json.supplyPumpFrequency) );
				//console.log( "this.json.supplyPumpFrequency-->"+ (parseInt(this.json.supplyPumpFrequency) >= 5) );
				//if( parseInt(this.json[heatUnitId+'-'+this.deviceId+'-supplyPumpFrequency']) >= 5.0 ){
				//	this.status = '运行';
				//}else{
					this.status = '未运行';
				//}
			}else if( parseInt(this.json[heatUnitId+'-'+this.deviceId+'-supplyPumpState']) >= 1 && (!supplyPumpFault || parseInt(supplyPumpFault) == 0 )){
				this.status = '运行';
			}else if( parseInt(this.json[heatUnitId+'-'+this.deviceId+'-supplyPumpState']) >= 1 && supplyPumpFault && parseInt(supplyPumpFault) >= 1 ){
				this.status = '故障';
			}
	　　} catch(err) {
			this.status = '未运行';
		...
		if( this.isPop == 1 && this.model == 'control' ){
			...
			if( this.isError == 0 ){
				...
				if( "/XMCP/plcControlStationStatusAction!queryRawPump.action" == this.win ){
					...
					$("#controlPage").css({top:(canvas.height-400)/2+50 ,left:(canvas.width-650)/2});
					$("#controlPage").css({width:650 ,height:360});
				}
	
				$("#controlPage").show(300);
			}else{
				alert('没有配置这个循环泵，不能控制！');
			}
