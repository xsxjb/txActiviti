...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			...
			<#list columns as po>
			   {field:'${po.fieldName}',title:'<#if po.filedComment?length lt 7 >${po.filedComment}<#else>${po.filedComment[0..6]}</#if>',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : '${po.classType_row}',
							options : {${po.optionType}}
					},
					formatter:function(value,row,index){
