...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : '${entityName?uncap_first}Action/datagrid.do',
		...
		${entityName?uncap_first}AddForm = $('#${entityName?uncap_first}AddForm').form({
			url : '${entityName?uncap_first}Action/add.do',
		...
		cdescAdd = $('#cdescAdd').xheditor({
			...
			html5Upload : true,
			upMultiple : 4,
			upLinkUrl : '${entityName?uncap_first}Action/upload.do',
			upLinkExt : 'zip,rar,txt,doc,docx,xls,xlsx',
			upImgUrl : '${entityName?uncap_first}Action/upload.do',
			upImgExt : 'jpg,jpeg,gif,png'
		});

		${entityName?uncap_first}EditForm = $('#${entityName?uncap_first}EditForm').form({
			url : '${entityName?uncap_first}Action/edit.do',
			success : function(data) {
				var json = $.parseJSON(data);
				if (json && json.success) {
					$.messager.show({
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${entityName?uncap_first}Action/delete.do',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
	...
	function edit() {
		...
		if (rows.length == 1) {
			...
			$.ajax({
				url : '${entityName?uncap_first}Action/showDesc.do',
				data : {
					...
					<#list columns as po>
						<tr>
							...
							<#if  po_index  lte field_required_num>
								<input name="${po.fieldName}" type="text" maxlength="${po.charmaxLength}" class="${po.classType}" data-options="${po.optionType}" missingMessage="请填写${po.filedComment}"  style="width: 155px;"/>
							<#else>
								<input name="${po.fieldName}" type="text" maxlength="${po.charmaxLength}" class="${po.classType}" data-options="" missingMessage="请填写${po.filedComment}"  style="width: 155px;"/>
	                    	</#if>
							</td>
						</tr>
					</#list>
